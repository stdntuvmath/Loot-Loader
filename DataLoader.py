...
def Run():
    """
    Main DataLoader loop for LootLoader.

    Responsibilities:
    - Pull historical minute + daily market data from Schwab
    - Calculate long-term indicators (EMA200, EMA angle, volatility bands, HLEV)
    - Persist calculated values into the LootLoader historical database
    - Periodically invoke the LootLoader decision logic per symbol
    - Handle recovery scenarios (auth failure, network failure, corrupted data)

    This function is intentionally long and procedural because it represents
    a time-ordered data ingestion pipeline rather than reusable business logic.
    """

    # Maximum number of symbols processed per cycle (ping-limit constrained)
    numberOfSymbols = 85
